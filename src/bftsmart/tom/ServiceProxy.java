...
public class ServiceProxy extends TOMSender {
     ...
     * @return The reply from the replicas related to request
     */
    public byte[] invokeOrderedCausal(byte[] request, byte[] causality) {
        return invokeCausal(request, causality, TOMMessageType.ORDERED_REQUEST);
    }
     ...
     * @return The reply from the replicas related to request
     */
    public byte[] invokeCausal(byte[] request, byte[] causality, TOMMessageType reqType) {
        ...
        try {
            ...
            if (requestType == TOMMessageType.UNORDERED_HASHED_REQUEST) {
                ...
                //TOMulticast(request, reqId, operationId, reqType);
                tomMulticastCausal(request, causality, reqId, operationId, reqType);
            }
