...
public class CommandsInfo implements Serializable {
	...
	public String toString() {
		...
		builder.append("CommandsInfo [ID=");
		builder.append(ID);
		builder.append("]");
	...
	public CommandsInfo(byte[][] commands, MessageContext[] msgCtx) {
		...
		if (msgCtx != null && msgCtx.length > 0) {
			...
			for (int i = 0; i < msgCtx.length; i++) {
				MessageContext msg = new MessageContext(msgCtx[i].getSender(), msgCtx[i].getViewID(),
						msgCtx[i].getType(), msgCtx[i].getSession(), msgCtx[i].getSequence(),
						msgCtx[i].getOperationId(), msgCtx[i].getReplyServer(), msgCtx[i].getSignature(),
						msgCtx[i].getTimestamp(), msgCtx[i].getNumOfNonces(), msgCtx[i].getSeed(),
						msgCtx[i].getRegency(), msgCtx[i].getLeader(), msgCtx[i].getConsensusId(), msgCtx[i].getProof(),
	...
	public boolean equals(Object obj) {
		if (obj instanceof CommandsInfo) {
			...
			if (this.commands != null && ci.commands != null) {
				...
				for (int i = 0; i < this.commands.length; i++) {
					...
					if (!(this.commands[i] == null && ci.commands[i] == null)
						...
						// System.out.println("[CommandsInfo] returing FALSE!5" + (this.commands[i] ==
			...
			// System.out.println((this.epoch == ci.epoch) + " " + (this.leader ==
			// ci.leader));
			return true;
		}
	...
	// had very large commands. Using the Serializable interface, the issue was
	// solved
	/*
	 * @Override public void writeExternal(ObjectOutput out) throws IOException {
	 * 
	 * if (commands != null) { out.writeInt(commands.length);
	 * 
	 * for (byte[] m : commands) { out.writeInt(m.length); out.write(m); } } else {
	 * out.writeInt(0); }
	 * 
	 * if (msgCtx != null) { out.writeInt(msgCtx.length);
	 * 
	 * for (MessageContext m : msgCtx) out.writeObject(m);
	 * 
	 * } else { out.writeInt(0); }
	 * 
	 * }
	 ...
	 * 
	 * int size = in.readInt(); if (size > 0) { commands = new byte[size][];
	 * 
	 * for (int i = 0; i < commands.length; i++) {
	 * 
	 * int length = in.readInt(); commands[i] = new byte[length];
	 * in.read(commands[i]); } }
	 * 
	 * size = in.readInt(); if (size > 0) {
	 * 
	 * msgCtx = new MessageContext[size]; for (int i = 0; i < commands.length; i++)
